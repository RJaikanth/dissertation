...
class SportsPosePreProcessor(PreprocessingBaseClass):
    ...
    def _get_trainval(self):
        ind_videos = glob.glob(os.path.join(self._root, "videos/indoors/**/*.avi"), recursive = True)
        out_videos = glob.glob(os.path.join(self._root, "videos/outdoors/**/*.avi"), recursive = True)
        ...
        valid_videos.extend(ind_valid_videos)
    ...
    def _get_bbox(self, joint, offset = 50):
        ...
        xyxy = [
            max(0, min_x - offset), max(0, min_y - offset),
            min(max_x + offset, 1216), min(1936, max_y + offset)
        ]
    ...
    def _extract_frame(self, sample, is_train):
        ...
        # Create annotation
        annotation = f"{cat_id} " + self._create_annotation(joints, bbox, crop_frame.shape)

        # Save files
        base_name = f"{self.curr_video.split('/')[-2]}"
        cv2.imwrite(os.path.join(self._OIROOT, is_train, f"{base_name}-{self.frame_counter:03d}.jpg"), transformed['image'])
        with open(os.path.join(self._OLROOT, is_train, f"{base_name}-{self.frame_counter:03d}.txt"), 'w') as f:
            f.write(annotation)
            f.close()

        # Log success
        print(f"{is_train:>5s} {base_name:>20s}: Frame-{self.frame_counter:03d}.jpg Saved")
        print(f"{is_train:>5s} {base_name:>20s}: Frame-{self.frame_counter:03d}.txt Saved")
